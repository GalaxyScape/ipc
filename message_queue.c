...
int main(void)
{
    ...
    int ret_1 = sem_init(&semx, 0, 0);
    int ret_2 = sem_init(&semy, 0, 0);
    int ret_3 = sem_init(&sem_rec, 0, 0);
    int ret_4 = sem_init(&sem_1, 0, 1);
    int ret_5 = sem_init(&sem_2, 0, 1);
    if (ret_1 == -1 || ret_2 == -1 || ret_3 == -1 || ret_4 == -1 || ret_5 == -1)
    {
        perror("sem_init");
        exit(-1);
    }
    ...
    {
        //sem_wait(&sem_rec);
        if (mq_receive(handler, echo, attr.mq_msgsize, NULL) == -1)
        {
            perror("mq_receive");
            exit(-1);
        }
        ...
        else if (strcmp(echo, end2) == 0)
        {
            if (sem_post(&semy) == -1)
            {
                perror("sem_post sem_y");
                exit(-1);
            }
            printf("Post semy\n");
            if (mq_send(handler, over2, strlen(over2), 0) == -1)
            {
                perror("mq_send");
                exit(-1);
            }
